...
Function PreReqCheck
	...
	OBSInstallerUtils::AddInUseFileCheck "$INSTDIR\data\plugins\win-capture\graphics-hook32.dll"
	OBSInstallerUtils::AddInUseFileCheck "$INSTDIR\data\plugins\win-capture\graphics-hook64.dll"
...
Section "OBS Studio" SecCore
	...
	SetOutPath "$INSTDIR"
	OBSInstallerUtils::KillProcess "plugins\32bit\cef-bootstrap.exe"
	OBSInstallerUtils::KillProcess "plugins\64bit\cef-bootstrap.exe"
	File /r "new\core\data"
	SetOutPath "$INSTDIR\bin"
	File /r "new\core\bin\32bit"
	SetOutPath "$INSTDIR\plugins"
	File /r "new\core\plugins\32bit"

	${if} ${RunningX64}
		SetOutPath "$INSTDIR\bin"
		File /r "new\core\bin\64bit"
		SetOutPath "$INSTDIR\plugins"
		File /r "new\core\plugins\64bit"
	${endif}
...
SectionGroup /e "Plugins" SecPlugins
	Section "Browser Source" SecPlugins_Browser
		...
		File /r "new\obs-browser\plugins\32bit"

		${if} ${RunningX64}
			OBSInstallerUtils::KillProcess "64bit\cef-bootstrap.exe"
			File /r "new\obs-browser\plugins\64bit"
		${endif}
	...
	Section /o "Realsense Source" SecPlugins_Realsense
		...
		SetOutPath "$INSTDIR\plugins"
		File /r "new\realsense\plugins\32bit"

		${if} ${RunningX64}
			File /r "new\realsense\plugins\64bit"
		${endif}

		SetOutPath "$INSTDIR\data\plugins"
		File /r "new\realsense\data\plugins\win-ivcam"

		ExecWait '"$INSTDIR\data\plugins\win-ivcam\seg_service.exe" /UnregServer'
		ExecWait '"$INSTDIR\data\plugins\win-ivcam\seg_service.exe" /RegServer'
...
Section "un.obs-studio Program Files" UninstallSection1
	...
	IfFileExists "$INSTDIR\data\plugins\win-ivcam\seg_service.exe" UnregisterSegService SkipUnreg
