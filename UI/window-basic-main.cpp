/******************************************************************************
    ...
    the Free Software Foundation, either version 2 of the License, orse
...
#include "features/video-list/video-list-window.hpp"
...
static void AddExtraModulePaths()
{
	...
	BPtr<char> config_bin = os_get_config_path_ptr("scap/plugins/%module%/bin");
	BPtr<char> config_data = os_get_config_path_ptr("scap/plugins/%module%/data");
...
static SelectScreenPosWindow* screenWindow = nullptr;
static SourcePreviewWindow* webcamWindow = nullptr;

bool windowMinimized = false;
VideoListWindow* videoListWindowRef;
	  ...
	  ui             (new Ui::OBSBasic)
{
	...
	// eddie additions
	ui->scenesDock->close();
	ui->transitionsDock->close();
	ui->mixerDock->setVisible(true);
	ui->addWebcamCapture->setEnabled(false);
	ui->recordButton->setEnabled(false);
	
	pauseButtonStopped();

	window = new FramelessWindow(nullptr);
	window->setMinimizeButtonVisible(true);
	QVBoxLayout *vLayout = new QVBoxLayout();

	timecode = new Timecode();//skyli commented
    //	timecode->setWindowFlag(Qt::Widget);//skyli added
	//  ui->verticalLayout->addWidget(timecode);//skyli added

	//skyli added
	//ui->controlsDock->layout()->addWidget(timecode);
	vLayout->addWidget(ui->controlsDock);
	vLayout->addWidget(timecode);
	window->setLayoutInsteadOfContent(vLayout);
	//skyli added
	//window->setContent(ui->controlsDock);//skyli changed
	window->setMinimumSize(0, 0);
	window->setSizePolicy(QSizePolicy::MinimumExpanding, QSizePolicy::MinimumExpanding);
	window->setAttribute(Qt::WA_DeleteOnClose, true);
	window->setAttribute(Qt::WA_NativeWindow, true);
	window->setWindowTitle("Screen Recorder v1.0");
	#ifdef __APPLE__
		ui->controlsDock->setStyleSheet("QToolButton:disabled { color: rgba(255,255,255,100); }");
	#endif

	QScreen* prim = QApplication::primaryScreen();
	window->show();//skyli changed
	window->setGeometry(
		prim->geometry().width() - window->width() - 35,
		prim->geometry().height() - window->height() - 35,
		window->width(),
		window->height()
	);
	...
	connect(window, &FramelessWindow::stateChanged, [this](QEvent*evt)
	{
		if(evt->type() == QEvent::WindowStateChange)
		{
			qDebug() << "FramelessWindow::stateChanged";
			...
			{
				if (outputHandler->RecordingActive()) { // "Pause"
					stoppedButLooksLikePaused = true;
					StopRecording();
				}
			}*/
			/*skyli added*/

			windowMinimized = this->window->windowState() == Qt::WindowState::WindowMinimized;
			SetupDesktopDisplay();


			if (this->window->windowState() == Qt::WindowState::WindowMinimized) { windowMinimizedHook(); }
			if (this->window->windowState() == Qt::WindowState::WindowActive) { windowRestoredHook(); }
		}
	});
		
	connect(window, SIGNAL(destroyed()), this, SLOT(close()));
	//skyli added
	connect(window, &FramelessWindow::aboutToClose, [this](QCloseEvent *ev){
		if (this->isVisible())
		...
		if (this->outputHandler && this->outputHandler->Active()) {
			...
			if (button == QMessageBox::Yes) {
				this->outputHandler->StopRecording();
				//this->window->close();
				ev->accept();
			}
			else {
	...
	connect(ui->addWebcamCapture, SIGNAL(released()), this, SLOT(AddWebcam()));
	connect(ui->addScreenRegion, SIGNAL(released()), this, SLOT(AddScreenRegion()));
	connect(ui->settingsButton, SIGNAL(released()), this, SLOT(SettingsButtonClicked()));

	connect(ui->previewButton, SIGNAL(released()), this, SLOT(PreviewButtonClicked()));
	connect(ui->uploadButton, SIGNAL(released()), this, SLOT(VideoListButtonClicked()));

	videoListWindowOpened = false;
}

void OBSBasic::AddWebcam()
{
	if(webcamWindow != nullptr) { delete webcamWindow; webcamWindow = nullptr; }

	ui->addWebcamCapture->setEnabled(false);

	obs_sceneitem_t* webcam = obs_scene_find_source(GetCurrentScene(), "webcam");

	if(webcam == nullptr)
	{
		obs_source_t* source = obs_source_create(WEBCAM_CAPTURE_ID, "webcam", NULL, nullptr);
		obs_properties_t* props = obs_get_source_properties(WEBCAM_CAPTURE_ID);
		obs_property* prop = obs_properties_get(props, WEBCAM_DEVICE_ID);
		size_t count = obs_property_list_item_count(prop);
		if (count == 0)
		{
			obs_source_release(source);
			(new QMessageBox(QMessageBox::Icon::Warning, "Error", "Couldn't find any webcams."))->exec();
		}
		else
		{
			const char* chosenWebcam;
			for (int i = 0; i < count; i++) {
				chosenWebcam = obs_property_list_item_string(prop, i);
				if (strlen(chosenWebcam) > 0) break;
			}

			qDebug() << "chosenWebcam" << chosenWebcam;
				
			obs_scene_add(GetCurrentScene(), source);
			obs_data_t* setting = obs_source_get_settings(source);
			obs_data_set_string(setting, WEBCAM_DEVICE_ID, chosenWebcam);
	...
	QTimer::singleShot(500, this,
		[=]( ) { ui->addWebcamCapture->setEnabled(true); }
	);
}

void OBSBasic::AddScreenRegion()
{
	...
	SelectScreenPosWindow screenRegion;

    // fixing no sizegrip
    FrameLess* f = new FrameLess(&screenRegion);
		f->setBorderWidth(7);
	
	screenRegion.activateWindow();
	screenRegion.setFocus();
	
	screenRegion.setAcceptKeyboardInput(true);
	screenRegion.setAcceptMouseInput(true);
	QRect scGeom = QApplication::primaryScreen()->geometry();
	screenRegion.setGeometry(0, 0, scGeom.width(), scGeom.height());

	if(screenRegion.exec() == QDialog::Accepted)
	{
		// deleting old screen region...
			...
			[](obs_scene_t *scene, obs_sceneitem_t *item, void *p)
		{
			...
			if(strcmp(id, DESKTOP_CAPTURE) == 0)
			{
		...
		QScreen* screen = screenRegion.hostScreen();
		QRect selectedRegion = screenRegion.geometry();
		QRect wholeScreen = screen->geometry();
		char name[150] = "screen";
			
		screenRegionChangedHook(selectedRegion);

		obs_source_t* source = obs_source_create(DESKTOP_CAPTURE, name, NULL, nullptr);
		
		
		obs_source_t* cropFilter = obs_source_create("crop_filter", "Crop Filter", NULL, nullptr);
		obs_source_filter_add(source, cropFilter);

		obs_data_t* cropFilterSettings = obs_source_get_settings(cropFilter);
		obs_data_set_int(cropFilterSettings, "cx", selectedRegion.width());
		obs_data_set_int(cropFilterSettings, "cy", selectedRegion.height());
		obs_data_set_int(cropFilterSettings, "left", selectedRegion.x() - wholeScreen.x());
		obs_data_set_int(cropFilterSettings, "top", selectedRegion.y() - wholeScreen.y());
		obs_data_set_bool(cropFilterSettings, "relative", false);
		obs_source_update(cropFilter, cropFilterSettings);
		obs_data_release(cropFilterSettings);


		for(int i = 0; i < QApplication::screens().size(); ++i)
		{
			if(QApplication::screens()[i] == screen)
			{
				obs_data_t* sourceSettings = obs_source_get_settings(source);

				obs_data_set_int(sourceSettings, SCREEN_ID, i);
				obs_data_set_int(sourceSettings, "desktop_x", selectedRegion.x());
				obs_data_set_int(sourceSettings, "desktop_y", selectedRegion.y());
				obs_data_set_int(sourceSettings, "desktop_w", selectedRegion.width());
				obs_data_set_int(sourceSettings, "desktop_h", selectedRegion.height());
		...
		changeVideoSize(selectedRegion.width(), selectedRegion.height());
		
		
		obs_sceneitem_t* sitem = obs_scene_add(GetCurrentScene(), source);
		SaveProject();
		obs_source_release(source);
		...
		pos.x = 0;
		pos.y = 0;
		obs_sceneitem_set_pos(sitem, &pos);

		ui->addWebcamCapture->setEnabled(true);
		ui->recordButton->setEnabled(true);
		sysTrayRecord->setEnabled(true);
...
void OBSBasic::changeVideoSize(uint width, uint height) {
	struct obs_video_info ovi;
	int ret;
	...
	AttemptToResetVideo(&ovi);
}

void OBSBasic::SettingsButtonClicked()
{
	if (isNewSettingsOpened)
		return;
	NewSettingsWindow *settings = new NewSettingsWindow;

	QPoint settingsPosition = window->geometry().translated(
		(settings->width() / 2),
		-(settings->height() + 10)
	).topLeft();

	settings->setAttribute(Qt::WA_DeleteOnClose, true);
	settings->move(settingsPosition);
	settings->setModal(false);//skyli edited
	/*skyli changed*/
	settings->show();//skyli edited
	...
	isNewSettingsOpened = true;
}

void OBSBasic::SetupDesktopDisplay()
{

	// clearing old windows.
	if(screenWindow != nullptr)
	{
		delete screenWindow;
		screenWindow = nullptr;
	}
	if(webcamWindow != nullptr)
	{
		delete webcamWindow;
		...
		[](obs_scene_t *scene, obs_sceneitem_t *item, void *p)
	{
		...
		if(strcmp(id, DESKTOP_CAPTURE) == 0)
		{
			...
			int x, y, w, h;
			x = obs_data_get_int(settings, "desktop_x") - 2;
			y = obs_data_get_int(settings, "desktop_y") - 2;
			w = obs_data_get_int(settings, "desktop_w") + 4;
			h = obs_data_get_int(settings, "desktop_h") + 4;

			SelectScreenPosWindow* window = new SelectScreenPosWindow;
			window->setAcceptMouseInput(false);
			window->setAcceptKeyboardInput(false);
			window->setText("");
		...
		else if(strcmp(id, WEBCAM_CAPTURE_ID) == 0)
		{
			//if(!windowMinimized)
			{
				SourcePreviewWindow* spw = new SourcePreviewWindow(item, nullptr);
				spw->setAttribute(Qt::WA_DeleteOnClose);
				spw->setAcceptKeyboardInput(false);

				FrameLess* f = new FrameLess(spw);
				f->setBorderWidth(7);
	...
	if(screenWindow != nullptr && webcamWindow != nullptr)
	{
		webcamWindow->setWindowBounds(screenWindow->geometry());//skyli changed
		// QScreen *screen = QGuiApplication::primaryScreen();//skyli added
		// QRect  screenGeometry = screen->geometry();//skyli added
		// webcamWindow->setWindowBounds(screenGeometry);//skyli added
	}
}


void OBSBasic::PreviewButtonClicked()
{
	setVisible(!isVisible());
}

void OBSBasic::VideoListButtonClicked()
{
	if (!videoListWindowOpened)
	{
		VideoListWindow *videoListWindow = new VideoListWindow();
		videoListWindowRef = videoListWindow;
		videoListWindow->setAttribute(Qt::WA_DeleteOnClose);
		
		videoListWindow->show();
		videoListWindowOpened = true;
	}
}
...
bool OBSBasic::InitBasicConfigDefaults()
{
	...
	config_set_default_uint  (basicConfig, "SimpleOutput", "VBitrate",
			1200);//bitrate skyli
	//config_set_default_uint(basicConfig, "SimpleOutput", "VBitrate", 2500);//bitrate skyli
...
void OBSBasic::OBSInit()
{
	...
	ret = snprintf(fileName, 512, "scap/basic/scenes/%s.json",
			sceneCollection);
	if (ret <= 0)
...
#undef SET_VISIBILITY
	...
	#endif // Disabled to avoid auto configuration.
...
void OBSBasic::RenderMain(void *data, uint32_t cx, uint32_t cy)
{
	...
	if (window->IsPreviewProgramMode()) {
		...
		{
			printf("Triggered an exception with message \"%s\".", ex.what());
		}
	} else {
...
int OBSBasic::AttemptToResetVideo(struct obs_video_info *ovi)
{
	int ret;
...
int OBSBasic::ResetVideo(uint widthOverride, uint heightOverride)
{
	if (outputHandler && outputHandler->Active())
	...
	if (widthOverride != 0 && heightOverride != 0) {
		ovi.base_width = (uint32_t)widthOverride;
		ovi.base_height = (uint32_t)heightOverride;
		ovi.output_width = (uint32_t)widthOverride;
		ovi.output_height = (uint32_t)heightOverride;
	}
...
{
	char logDir[512];
	if (GetConfigPath(logDir, sizeof(logDir), "scap/logs") <= 0)
		return nullptr;
...
void OBSBasic::UploadLog(const char *file)
{
	...
	data_t windowWidget{obs_data_create(), data_deleter};
	data_t files{obs_data_create(), data_deleter};
	data_t request{obs_data_create(), data_deleter};

	obs_data_set_string(windowWidget.get(), "content", fileString);

	obs_data_set_obj(files.get(), file, windowWidget.get());
...
{
	if (!(outputHandler->RecordingActive()) && !stoppedButLooksLikePaused) {
		recordingStartingHook();
		StartRecording(); // "Record"
	} else if (outputHandler->RecordingActive() && !stoppedButLooksLikePaused) {
		StopRecording(); // "Stop" from recording state
		if (videoListWindowOpened)
			videoListWindowRef->displayVideos();

	} else if (!(outputHandler->RecordingActive()) && stoppedButLooksLikePaused) {
		stoppedButLooksLikePaused = false;
		recordButtonStopped();
...
void OBSBasic::on_pauseButton_clicked()
{
	if (outputHandler->RecordingActive()) { // "Pause"
		stoppedButLooksLikePaused = true;
		StopRecording();
...
{
	stoppedButLooksLikePaused = false; // reset
...
void OBSBasic::StopRecording()
{
	...
	if (outputHandler->RecordingActive())
		outputHandler->StopRecording(true);//skyli changed

	OnDeactivate();
}


void OBSBasic::recordingStartingHook() {
	...
	timecode->start();//skyli commented
	
	manageVisibility(true);
	
	ui->addScreenRegion->setEnabled(false);
}

void OBSBasic::recordingPausedHook() {
	pauseButtonPaused();
	addFileToVideoStitcher();
	
	timecode->pause();//skyli commented
	
	manageVisibility(false);
	
	ui->addScreenRegion->setEnabled(true);
}

void OBSBasic::recordingResumedHook() {
	timecode->resume();//skyli commented
...
void OBSBasic::recordingStoppedHook() {
	pauseButtonStopped();
	//QString videoFile = QString::fromStdString(outputHandler->lastFilenameUsed);
	
	addFileToVideoStitcher();//skyli changed
	startStitching();
	
	timecode->stop();//skyli commencted
...
void OBSBasic::screenRegionChangedHook(QRect screenRegion) {
	timecode->setRestrictedArea(screenRegion);//skyli changed
	restrictedArea = screenRegion;
}

void OBSBasic::manageVisibility(bool recording) {
	if (recording && restrictedArea.intersects(window->geometry())) {
...
void OBSBasic::windowMinimizedHook() {
	if (screenWindow != nullptr) 
	{
		//screenWindow->hide();
	}
...
void OBSBasic::windowRestoredHook() {
	if (screenWindow != nullptr) { screenWindow->show(); }
}
...
void OBSBasic::ToggleShowHide()
{
	if (window->isVisible()) {
		...
		windowMinimizedHook();
		sysTrayShowHide->setText("Show");
	} else {

		window->show();
		if (webcamWindow != nullptr) { webcamWindow->show(); }

		windowRestoredHook();
		sysTrayShowHide->setText("Hide");
	}
}


void OBSBasic::resetState() {
	if(screenWindow != nullptr) {
		delete screenWindow;
		screenWindow = nullptr;
	}
	...
	obs_sceneitem_t* screen = obs_scene_find_source(GetCurrentScene(), "screen");
	if (screen != nullptr) { obs_sceneitem_remove(screen); }

	obs_sceneitem_t* webcam = obs_scene_find_source(GetCurrentScene(), "webcam");
	if (webcam != nullptr) { obs_sceneitem_remove(webcam); }

	
	ui->addWebcamCapture->setEnabled(false);
	ui->recordButton->setEnabled(false);
	sysTrayRecord->setEnabled(false);
}


void OBSBasic::addFileToVideoStitcher() {
	QString videoFile = QString::fromStdString(outputHandler->lastFilenameUsed);
	if (!videoStitcher->isFileAdded(videoFile))
		videoStitcher->addFile(videoFile);
}

void OBSBasic::startStitching() {
	ui->uploadButton->setEnabled(false);
	connect(videoStitcher, SIGNAL(finished()), this, SLOT(StitchingFinished()));
	videoStitcher->start();
}

void OBSBasic::StitchingFinished() {
	ui->uploadButton->setEnabled(true); 
	videoStitcher->deleteLater();
}


void OBSBasic::pauseButtonRecording() {
	ui->pauseButton->setVisible(true);
	ui->pauseButton->setText("Pause");
	ui->pauseButton->setIcon(QIcon(":/images/new/pause.png"));
	
	sysTrayPause->setVisible(true);
	sysTrayPause->setText("Pause");
}

void OBSBasic::pauseButtonPaused() {
	ui->pauseButton->setVisible(true);
	ui->pauseButton->setText("Resume");
	ui->pauseButton->setIcon(QIcon(":/images/new/continue.png"));
	
	sysTrayPause->setVisible(true);
	sysTrayPause->setText("Resume");
}

void OBSBasic::pauseButtonStopped() {
	ui->pauseButton->setVisible(false);
	if (sysTrayPause) { sysTrayPause->setVisible(false); }
}



void OBSBasic::recordButtonStopping()
{
...
void OBSBasic::recordButtonRecording()
{
	...
	ui->recordButton->setIcon(QIcon(":/images/new/stop.png"));
...
void OBSBasic::recordButtonStopped(int code)
{
	if (!stoppedButLooksLikePaused) {
		ui->statusbar->RecordingStopped();
		ui->recordButton->setText("Record");
		ui->recordButton->setIcon(QIcon(":/images/new/record.png"));		
	}

	if (stoppedButLooksLikePaused) {
		recordingPausedHook();
	} else {
		recordingStoppedHook();
	}
...
{
	char path[512];
	int ret = GetConfigPath(path, 512, "scap");
	if (ret <= 0)
		return;
...
int OBSBasic::GetProfilePath(char *path, size_t size, const char *file) const
{
	...
	ret = GetConfigPath(profiles_path, 512, "scap/basic/profiles");
	if (ret <= 0)
		return ret;
...
void OBSBasic::SetShowing(bool showing)
{
	if (!showing && isVisible()) {
		...
		if (sysTrayShowHide)
			sysTrayShowHide->setText(QTStr("Basic.SystemTray.Show"));
		...
		if (sysTrayShowHide)
			sysTrayShowHide->setText(QTStr("Basic.SystemTray.Hide"));
...
void OBSBasic::SystemTrayInit()
{
	...
	trayIcon->setToolTip("Screen Recorder");

	sysTrayShowHide = new QAction(QTStr("Basic.SystemTray.Hide"), trayIcon);
	sysTrayRecord = new QAction(QTStr("Basic.Main.StartRecording"), trayIcon);
	sysTrayPause = new QAction("Pause", trayIcon);
	sysTrayReplayBuffer = new QAction(QTStr("Basic.Main.StartReplayBuffer"), trayIcon);
	sysTrayExit = new QAction(QTStr("Exit"), trayIcon);

	if (outputHandler && !outputHandler->replayBuffer)
		sysTrayReplayBuffer->setEnabled(false);
	
	sysTrayRecord->setEnabled(false); // Record button initially disabled
	
	// connect(trayIcon, SIGNAL(activated(QSystemTrayIcon::ActivationReason)), this, SLOT(IconActivated(QSystemTrayIcon::ActivationReason)));
	connect(sysTrayShowHide, SIGNAL(triggered()), this, SLOT(ToggleShowHide()));
	...
	connect(sysTrayPause, SIGNAL(triggered()), this, SLOT(on_pauseButton_clicked()));
	// connect(sysTrayReplayBuffer.data(), &QAction::triggered, this, &OBSBasic::ReplayBufferClicked);
	connect(sysTrayExit, SIGNAL(triggered()), this, SLOT(ExitButtonClicked()));
	...
	trayMenu = new QMenu;
	trayMenu->addAction(sysTrayShowHide);
	// trayMenu->addMenu(previewProjector);
	// trayMenu->addAction(sysTrayStream);
	trayMenu->addAction(sysTrayRecord);
	trayMenu->addAction(sysTrayPause);
	// trayMenu->addAction(sysTrayReplayBuffer);
	trayMenu->addAction(sysTrayExit);
	trayIcon->setContextMenu(trayMenu);
	
	pauseButtonStopped(); // will configure sysTrayPause
}
		...
		QSystemTrayIcon::MessageIcon n)
{
	if (QSystemTrayIcon::supportsMessages()) {
		QSystemTrayIcon::MessageIcon icon =
				QSystemTrayIcon::MessageIcon(n);
		trayIcon->showMessage("Screen Recorder", text, icon, 10000);
	}
}
