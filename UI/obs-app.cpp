...
static bool MakeUserDirs()
{
	...
	if (GetConfigPath(path, sizeof(path), "scap/basic") <= 0)
		return false;
	if (!do_mkdir(path))
		return false;

	if (GetConfigPath(path, sizeof(path), "scap/logs") <= 0)
		return false;
	if (!do_mkdir(path))
		return false;

	if (GetConfigPath(path, sizeof(path), "scap/profiler_data") <= 0)
		return false;
	if (!do_mkdir(path))
		return false;

#ifdef _WIN32
	if (GetConfigPath(path, sizeof(path), "scap/crashes") <= 0)
		return false;
	if (!do_mkdir(path))
		return false;

	if (GetConfigPath(path, sizeof(path), "scap/updates") <= 0)
		return false;
	if (!do_mkdir(path))
		return false;
#endif

	if (GetConfigPath(path, sizeof(path), "scap/plugin_config") <= 0)
		return false;
	if (!do_mkdir(path))
		return false;
...
static bool MakeUserProfileDirs()
{
	...
	if (GetConfigPath(path, sizeof(path), "scap/basic/profiles") <= 0)
		return false;
	if (!do_mkdir(path))
		return false;

	if (GetConfigPath(path, sizeof(path), "scap/basic/scenes") <= 0)
		return false;
	if (!do_mkdir(path))
		return false;
...
static void move_basic_to_profiles(void)
{
	...
	if (GetConfigPath(path, 512, "scap/basic") <= 0)
		return;
	if (!os_file_exists(path))
		return;

	/* if the profiles directory doesn't already exist */
	if (GetConfigPath(new_path, 512, "scap/basic/profiles") <= 0)
		return;
...
static void move_basic_to_scene_collections(void)
{
	...
	if (GetConfigPath(new_path, 512, "scap/basic/scenes") <= 0)
		return;
	if (os_file_exists(new_path))
		return;
...
{
	BPtr<char>       logDir(GetConfigPathPtr("scap/logs"));
	struct os_dirent *entry;
	os_dir_t         *dir        = os_opendir(logDir);
...
static void create_log_file(fstream &logFile)
{
	...
	dst << "scap/logs/" << currentLogFile.c_str();
...
#define LITERAL_SIZE(x) x, (sizeof(x) - 1)
	ostringstream dst;
	dst.write(LITERAL_SIZE("scap/profiler_data/"));
...
static int run_program(fstream &logFile, int argc, char *argv[])
{
	...
	try {
		...
		delete_oldest_file("scap/profiler_data");
		...
		if (already_running && !multi) {
			blog(LOG_WARNING, "\n================================");
			blog(LOG_WARNING, "Warning: Screen Recorder is already running!");
			blog(LOG_WARNING, "================================\n");
...
#define CRASH_MESSAGE \
	"Woops, Screen Recorder has crashed!\n\nWould you like to copy the crash log " \
...
static void main_crash_handler(const char *format, va_list args, void *param)
{
	...
	delete_oldest_file("scap/crashes");

	string name = "scap/crashes/Crash ";
	...
	file.close();
	//skyli changed
	int ret = MessageBoxA(NULL, CRASH_MESSAGE, "Screen Recorder Error!",
...
static void move_to_xdg(void)
{
	...
	if (GetConfigPath(new_path, 512, "scap") <= 0)
		return;
...
{
	char path[512];
	int pathlen = GetConfigPath(path, 512, "scap/basic/profiles");
