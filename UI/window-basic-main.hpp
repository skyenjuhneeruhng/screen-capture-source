...
#include "features/timecode/timecode.hpp"
#include "features/pause/video-stitcher.hpp"
#include "features/countdown/countdown.hpp"
...
#ifdef _WIN32
	#define DESKTOP_CAPTURE "monitor_capture"
	#define SCREEN_ID "monitor"
	#define WEBCAM_CAPTURE_ID "dshow_input"
	#define WEBCAM_DEVICE_ID "video_device_id"
#elif __APPLE__
	#define DESKTOP_CAPTURE "display_capture"
	#define SCREEN_ID "display"
	#define WEBCAM_CAPTURE_ID "av_capture_input"
	#define WEBCAM_DEVICE_ID "device"
#endif
...
private:
	...
	QPointer<QAction>         sysTrayShowHide;
	QPointer<QAction>         sysTrayExit;
	...
	QList<SelectScreenPosWindow*> m_windows;
	...
	QList<SelectScreenPosWindow*> m_screenPosWindows;
...
public:
	//skyli added
	bool videoListWindowOpened;


public slots:
	void AddWebcam();
	void AddScreenRegion();
	void SettingsButtonClicked();
	void PreviewButtonClicked();
	void VideoListButtonClicked();
	void StartStreaming();
	void StopStreaming();
	...
	void recordButtonRecording();
	void recordButtonStopped(int code = OBS_OUTPUT_SUCCESS);
	void recordButtonStopping();

	void recordingStartingHook();
	void recordingPausedHook();
	void recordingResumedHook();
	void recordingStoppedHook();
	
	void screenRegionChangedHook(QRect);
	void windowMinimizedHook();
	void windowRestoredHook();

	void addFileToVideoStitcher();
	void startStitching();
	void StitchingFinished();
	
	void resetState();
	void changeVideoSize(uint, uint);
...
private:
	...
	void pauseButtonRecording();
	void pauseButtonPaused();
	void pauseButtonStopped();
	
	void manageVisibility(bool);
	
	bool stoppedButLooksLikePaused = false;
...
public:
	bool isNewSettingsOpened = false;
	...
	int  ResetVideo(uint widthOverride = 0, uint heightOverride = 0);
	bool ResetAudio();
